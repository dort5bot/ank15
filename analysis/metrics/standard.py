...
class YAMLStandard:
    ...
    @staticmethod
    # Mevcut koda eklenebilir

    def enhanced_create_metric_yaml(
        technical_name: str,
        category: str,
        description: str,
        formula: str = "",
        ...
        unit: str = "score",
        parameters: Dict[str, Dict] = None,  # Gelişmiş parametre yapısı
        ...
        """Enhanced YAML structure with all new fields"""
        ...
        # ✅ Optional fields - sadece tanımlanmışsa ekle
        if dependencies:
            metric_data["dependencies"] = dependencies
            
        if value_range:
            metric_data["range"] = value_range
            
        if interpretation:
            metric_data["interpretation"] = interpretation
...
def metric_standard(
    ...
    safe: bool = True,
    apply_output_shape: bool = True,
    preserve_index: bool = True  # Yeni: Index koruma opsiyonu
):
    """
    Gelişmiş metric standardizasyon dekoratörü.
    
    Yeni özellikler:
    - Input/output index uyumu
    - Multi-input için dict output hizalama  
    - Metadata tam koruma
    - Graceful error recovery
    """
    
    def decorator(func):
        @functools.wraps(func)  # ✅ Metadata otomatik koruma
        def wrapper(*args, **kwargs):
            try:
                ...
                # 2. Orijinal input metadata'sını sakla (index, shape, etc.)
                original_metadata = None
                if preserve_index and not multi_input and hasattr(data, 'index'):
                    original_metadata = {
                        'index': getattr(data, 'index', None),
                        'shape': getattr(data, 'shape', None),
                        'name': getattr(data, 'name', None)
                    }
                elif preserve_index and multi_input and isinstance(data, dict):
                    original_metadata = {}
                ...
                if multi_input:
                    if not isinstance(data, dict):
                        raise TypeError("multi_input=True requires dict input")
                    standardized_data = {
                        k: MetricStandard.standardize_input(v, input_type, fillna, min_periods)[0]
                        for k, v in data.items()
                    }
                else:
                    standardized_data, _ = MetricStandard.standardize_input(
                        data, input_type, fillna, min_periods
                    )

                # 4. Validate
                if not MetricStandard.validate_input(standardized_data, min_periods):
                    return _safe_nan_output(output_type, original_metadata)

                # 5. Update args and call function
                if args:
                ...
                # 6. Standardize output with shape preservation
                output = MetricStandard.standardize_output(result, output_type, standardized_data)
                
                # 7. Apply shape alignment if requested
                if apply_output_shape and original_metadata:
                    output = _apply_shape_alignment(output, original_metadata, multi_input)
...
def _apply_shape_alignment(output, original_metadata, multi_input):
    """Output shape'ini original input'a hizala"""
    try:
        if not multi_input and isinstance(output, np.ndarray):
            # Numpy array -> Pandas Series with original index
            if 'index' in original_metadata and original_metadata['index'] is not None:
                return pd.Series(output, index=original_metadata['index'])
        
        elif multi_input and isinstance(output, dict):
            # Dict output için her key'i hizala
            aligned = {}
            for k, v in output.items():
                if k in original_metadata and isinstance(v, np.ndarray):
                    meta = original_metadata[k]
                    if meta and meta.get('index') is not None:
                        aligned[k] = pd.Series(v, index=meta['index'])
                    else:
                        aligned[k] = v
                else:
                    aligned[k] = v
        ...
        # Shape alignment optional, fail gracefully
        pass
    
    return output

def _safe_nan_output(output_type, original_metadata):
    """Güvenli NaN output with shape preservation"""
    nan_val = np.nan
    if output_type == "pandas" and original_metadata and 'index' in original_metadata:
        # Pandas Series with NaN and original index
        return pd.Series([nan_val], index=[original_metadata['index'][-1] if original_metadata['index'] else 0])
    return nan_val
